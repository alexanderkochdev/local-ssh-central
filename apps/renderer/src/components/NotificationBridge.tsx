  ...
  useEffect(() => {
    ...
    const offSftp = window.api.onEvent(IpcChannels.sftpEvent, (payload) => {
      ...
      if (event.type === 'connectionClosed') {
        // Verbindung unerwartet beendet: im Hauptfenster informieren (Fenster schliesst der Main).
        dispatch(pushNotification({ type: 'info', title: 'SFTP-Verbindung beendet', autoDismissMs: 3000 }));
      } else if (sftp && event.type === 'transferProgress') {
        void dispatch(handleSftpTransfer(event.transfer));
      } else if (sftp && event.type === 'transferBatchTotal') {
